...
{ int d;
  ...
  for (d=0; d<4; d++)
    if (!EMPTY(ADJ(gn,d)))
      printf(" (%d,%d)",POINT_X(ADJ(gn,d)),POINT_Y(ADJ(gn,d)));
    else
      printf("  --- ");
  putchar(10);
}
...
{ int d;
  for (d=0; d<4; d++)
    if (!EMPTY(ADJ(gn,d)))
      //printf("%c %d %d %d %d\n", Windrose[d], POINT_X(gn), POINT_Y(gn), POINT_X(ADJ(gn,d)),POINT_Y(ADJ(gn,d)));
...
{ 
  ...
  sweep(hedges,SOUTH);
  //  printf("%d %d\n",n+(n-4)/2,8*(n-4)/2+8);  
  // No.vertices and hedges of H-Part
  // Euler Th:  F+V = E+1   E = (r+1) + (2r+4)+r-1 = 4r+4 

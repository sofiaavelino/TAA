...
class FaceComparator implements Comparator<Face> {
    ...
    public int compare(Face f1, Face f2) {
        Vertex c1 = f1.centroid;
        Vertex c2 = f2.centroid;
        if (c1.y != c2.y) {
...
class DCEL {
    ...
    public void iterateThroughEdges() {
        ...
        for (int i = 0; i < halfEdges.size() / 2; i++) {
            ...
            while (h != h1) {
                ...
                // System.out.println("Twin Origin: (" + h.twin.origin.x + ", " +
                // h.twin.origin.y + ")");
    ...
    public void generateHull(int k, DCEL copy) {
        List<Vertex> modemEdges = new ArrayList<>();
        List<Face> modemFaces = new ArrayList<>();
        for (Face f : faces) {
            // System.out.println("Face avaliada " + f.counter + " com modem " + f.modem);
            if (f.modem <= k && f.counter != 0) {
                // System.out.println("Entrei");
                modemFaces.add(f);
                HalfEdge start_e = f.outerComponent;
                if (start_e.twin.incidentFace.modem > k || start_e.twin.incidentFace.counter == 0) {
                    // System.out.println("VERTEX ADICIONADO AO HULL:" + start_e.origin.x + " " +
                    // start_e.origin.y);
                    modemEdges.add(start_e.origin);
                }
                HalfEdge e = start_e.next;
                // System.out.println("Start edge = " + start_e.origin.x + " " +
                // start_e.origin.y);
                while (start_e != e) {
                    // System.out.println("E edge = " + e.origin.x + " " + e.origin.y);
                    if (e.twin.incidentFace.modem > k || e.twin.incidentFace.counter == 0) {
                        // System.out.println("ESTE VERTEX VAI SER ADICIONADO //" + e.origin.x + " " +
                        // e.origin.y);
                        modemEdges.add(e.origin);
                    }
                    e = e.next;
                }
            }
        }
        List<Vertex> almostReady = new ArrayList<>();
        // System.out.println("K = " + k);
        for (HalfEdge e : copy.externalEdges) {
            if (modemEdges.contains(e.origin)) {
                // System.out.println("Vertice " + e.origin.x + " " + e.origin.y);
                almostReady.add(e.origin);
            }
        }

        Vertex[] vertices = new Vertex[externalVertices.size()];
        int i = 0;
        for (Vertex e : copy.externalVertices) {
            vertices[i] = e;
            i++;
        }
        DCEL hull = new DCEL();
        hull.createDCELFromPolygon(vertices);

        List<Vertex> ordered = new ArrayList<>();
        for (HalfEdge halfedge : hull.halfEdges) {
            // System.out.println("Vertex " + halfedge.origin.x + " " + halfedge.origin.y +
            // " " + halfedge.next.origin.x
            // + " " + halfedge.next.origin.y);
            if (modemEdges.contains(halfedge.origin)) {
                if (modemEdges.contains(halfedge.next.origin)) {
                    ordered.add(halfedge.origin);
                    ordered.add(halfedge.next.origin);
                    continue;
                } else {
                    for (Vertex v : almostReady) {
                        if (halfedge.origin.x == halfedge.next.origin.x && halfedge.origin.x == v.x) {
                            if ((v.y <= halfedge.origin.y && v.y >= halfedge.next.origin.y)
                                    || (v.y >= halfedge.origin.y && v.y <= halfedge.next.origin.y)) {

                                // System.out.println(v.x + " " + v.y + "Vertex " + halfedge.origin.x + " " +
                                // halfedge.origin.y +
                                // " " + halfedge.next.origin.x
                                // + " " + halfedge.next.origin.y);
                                ordered.add(v);
                            }
                        } else if (halfedge.origin.y == halfedge.next.origin.y && halfedge.origin.y == v.y) {
                            if ((v.x <= halfedge.origin.x && v.x >= halfedge.next.origin.x)
                                    || (v.x >= halfedge.origin.x && v.x <= halfedge.next.origin.x)) {
        ...
        // seen.add(ordered.getFirst());
        for (int ii = ordered.size() - 1; ii > 0; ii--) {
            Vertex currentElement = ordered.get(ii);
            if (seen.contains(currentElement)) {
                ordered.remove(ii);
            } else {
                seen.addFirst(currentElement);
            }
        }
        System.out.println("FINAL com k = " + k);
        for (Vertex v : seen) {
            System.out.println(v.x + " " + v.y);
        }
    }

    public void mergeFaces(int k) {
        List<Face> modemFaces = new ArrayList<>();
        ...
        Collections.sort(modemFaces, new FaceComparator()); // quero começar da face mais a cima (ou mais à direita em
                                                            // caso de empate) para
        // garantir que tenho pelo menos 1 fronteira com uma face de modem > k
        int region = 0;

        while (modemFaces.size() != 0) {
            System.out.println("Size = " + modemFaces.size());
            region++;
            ...
            System.out.println("Start = " + startE.origin.x + " " + startE.origin.y);
            while (isBarrier == false) {
                if (startE.next.twin.incidentFace.counter != 0) {
                    isBarrier = true;
                    break;
                }
                startE = startE.next;
            ...
            while (e.origin != startE.origin) {
                if (e.twin.incidentFace.modem <= k) {
                    System.out.println(
                            "Edge = " + e.origin.x + " " + e.origin.y + " " + e.next.origin.x + " "
                                    + e.next.origin.y);
            ...
            System.out.print("Region " + region + ": (" + startE.origin.x + "," + startE.origin.y + ") ");
            e = startE.next;
            while (e != startE) {
                System.out.print(e.origin.x + "," + e.origin.y + ") ");
                e = e.next;
            }
            System.out.println();
